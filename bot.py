...
async def spank(ctx, userName: discord.Member = None):
    ...
    msg.set_footer(text=footer_text)
    if userName == None:
        msg.add_field(name=":octagonal_sign: ", value="`}spank <user>`")
    else:
        msg.set_image(url="{}".format(random.choice(spanklinks)))
        msg.add_field(name=":handshake: Interactions", value="`{} spanked {}!`".format(author.display_name, userName.display_name), inline=True)
    await client.say(embed=msg)
    print("============================================================")
    print("}spank <user>")
    print("{} ### {}".format(author, author.id))
